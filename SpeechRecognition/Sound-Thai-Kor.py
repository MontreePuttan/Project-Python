...
def translate_and_convert_to_mp3(text, dest_language='ko'):
    # Translate the text to Korean
    translated = translator.translate(text, dest=dest_language)
    print(f"Original (Thai): {text}")
    print(f"Translated (Korean): {translated.text}")
...
# Main function to recognize Thai speech and translate to Korean
def main():
    print("== เริ่มการทำงาน ==")
    with sr.Microphone() as source:
        ...
        try:
            ...
            # Translate the recognized text from Thai to Korean and convert to MP3
            translate_and_convert_to_mp3(text, dest_language='ko')
